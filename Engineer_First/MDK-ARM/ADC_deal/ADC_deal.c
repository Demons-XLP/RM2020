...
* @par 
* @par     这个文件的输入为ADC1_DMA中断启动函数处得到的ADC_Value[400]
* @par     最后处理的结果为final_ADC_volage[4]
* @par     最后夏普红外拟合数据为ADC1_Sharp_Distance、ADC2_Sharp_Distance
*				使用方法：1.在头文件中定义自己想要的ADC1/2
                  2.将初始化函数ADC_Sensor_Init()初始化即可
...
#ifdef ADC_USE_ADC1
uint16_t ADC1_Value[400];   //存储器中采集到的数据 
uint16_t ADC2_Value[400];   //存储器中采集到的数据 
//分离出单个通道采集到的数据
...
uint16_t final_ADC1_volage[4];  // 最后处理出来的电压结果
float    ADC1_Sharp_Distance[4];    // 夏普红外测距，经过拟合的
...
uint16_t final_ADC2_volage[4];  // 最后处理出来的电压结果
float    ADC2_Sharp_Distance[4];    // 夏普红外测距，经过拟合的
#endif
...
#ifdef ADC_USE_ADC1
	if(hadc == &hadc1){
		get_4ch(ADC1_Value,ADC1_ch1_value,400,0);
		get_4ch(ADC1_Value,ADC1_ch2_value,400,1);
		get_4ch(ADC1_Value,ADC1_ch3_value,400,2);
		get_4ch(ADC1_Value,ADC1_ch4_value,400,3);
		...
		for(int i=0; i<4; i++){
			...
			ADC1_ch_sum[i] /= 10.0f;	
			final_ADC1_volage[i] =  (uint16_t) (ADC1_ch_sum[i] - ADC1_ch_offset[i]);    
		}
		for(uint8_t i=0;i<2;i++){
			if(final_ADC1_volage[i] < 750)  final_ADC1_volage[i] = 750;  // 对应60cm左右
			ADC1_Sharp_Distance[i] = 284486*pow(final_ADC1_volage[i],-1.275);
		}
...
#ifdef ADC_USE_ADC2
	if(hadc == &hadc2){
		get_4ch(ADC2_Value,ADC2_ch1_value,400,0);
		get_4ch(ADC2_Value,ADC2_ch2_value,400,1);
		get_4ch(ADC2_Value,ADC2_ch3_value,400,2);
		get_4ch(ADC2_Value,ADC2_ch4_value,400,3);
		...
		for(int i=0; i<4; i++){
			...
			ADC2_ch_sum[i] /= 10.0f;	
			final_ADC2_volage[i] =  (uint16_t) (ADC2_ch_sum[i] - ADC2_ch_offset[i]);    
		}
...
{
	#ifdef ADC_USE_ADC1
 	HAL_ADC_Start_DMA(&hadc1,(uint32_t*)ADC1_Value,400);//红外激光测距数据采集
	#endif
	#ifdef ADC_USE_ADC2
  HAL_ADC_Start_DMA(&hadc2,(uint32_t*)ADC2_Value,400);//红外激光测距数据采集
	#endif
}
