...
>>>>>>> 32bd7eed52aa9488da37e8424c0c7523145e7274
*/  
...
WEAK void motor::update(uint8_t Data[])
{
	...
	LastCurrentEncoder = CurrentEncoder;//保存转矩电流
	RealPosition = Data[0]<<8 | Data[1];
	RealSpeed = Data[2]<<8 | Data[3];
	CurrentEncoder = Data[4]<<8 | Data[5];
	if(CurrentEncoder != LastCurrentEncoder)//前后转矩电流不同才算作有效数据
...
WEAK void chassiscontrol::Run(int16_t Vx, int16_t Vy, int16_t Omega, uint8_t Mode, 
	uint8_t Reverse, uint8_t SuperCap)
{
	Last_Vx=Vx;
	Last_Vy=Vy;
	Last_Omega=Omega;
	...
	Reverse = 0x02&(Reverse<<1);
	SuperCap &= 0x01;
	
	int16_t sendbuf[4] = {0};
	sendbuf[0] = (Mode<<8) | (Reverse|SuperCap);
	sendbuf[1] = Vx;
	sendbuf[2] = Vy;
	sendbuf[3] = Omega;
